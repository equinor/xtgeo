...
x_plane_normalvector(double *points_v, double *nvector, int option)
{
    ...
    d = -1 * (x1 * (y2 * z3 - y3 * z2) + x2 * (y3 * z1 - y1 * z3) +
              x3 * (y1 * z2 - y2 * z1));
...
x_isect_line_plane(double *nvector, double *line_v, double *point_v, int option)
{
    ...
    if (option == 2 && (u < 0.0 || u > 1.0)) {
        ...
        throw_exception("Line does not cross the plane");
        return (2);
    }
...
x_sample_z_from_xy_cell(double *cell_v, double x, double y, int option, int option2)
{
    ...
    insidecell = pol_chk_point_inside(x, y, px, py, 5);
    ...
    inside = pol_chk_point_inside(x, y, px, py, 4); /* check for triangle */
