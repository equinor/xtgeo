...

{
    ...
    if (fc == NULL) {
        throw_exception("Could not open file: %s", filename);
        return;
    }
    /*
     *-------------------------------------------------------------------------
    ...
    grd3d_write_eclrecord(fc, "FILEHEAD", 1, itmp, &fdum, &ddum, 100);
    ...
    grd3d_write_eclrecord(fc, "ENDGRID", 1, itmp, &fdum, &ddum, 1);
