                      ...
                      int option)
{
    ...
    logger_info(LI, FI, FU, "Write Storm binary map file ... (%s)", __FUNCTION__);
    ...
    for (i = 0; i < mxy; i++) {
        ...
        if (swap == 1)
            SWAP_DOUBLE(dbl_value);

        if (fwrite(&dbl_value, 8, 1, fc) != 1) {
            logger_error(LI, FI, FU, "Error writing to Storm format. Bug?");
            return -1;
        }
    }
