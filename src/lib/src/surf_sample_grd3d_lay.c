...

{
    ...
    if (x_verify_vectorlengths(nx, ny, nz, ncoord, nzcorn, ntot, 1, XTGFORMAT1) != 0) {
    ...
    for (j = 1; j <= ny; j++) {
        for (i = 1; i <= nx; i++) {
            ...
            for (ii = 0 + ishift; ii <= 9 + ishift; ii += 3) {
                if (corners_v[ii] < cxmin)
                    cxmin = corners_v[ii];
                if (corners_v[ii] > cxmax)
                    cxmax = corners_v[ii];
            }
            ...
            for (ii = 1 + ishift; ii <= 10 + ishift; ii += 3) {
                if (corners_v[ii] < cymin)
                    cymin = corners_v[ii];
                if (corners_v[ii] > cymax)
                    cymax = corners_v[ii];
            }
