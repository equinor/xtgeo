...

{
    ...
    kshift = 0;
    if (koption == 1)
        kshift = 12; /* lower cell layer, not upper */
    ...
    for (i = 1; i <= nx; i++) {
        for (j = 1; j <= ny; j++) {
            ...
            slicev[icx++] = crs[0 + kshift];
            slicev[icx++] = crs[1 + kshift];
            slicev[icx++] = crs[3 + kshift];
            slicev[icx++] = crs[4 + kshift];
            slicev[icx++] = crs[9 + kshift];
            slicev[icx++] = crs[10 + kshift];
            slicev[icx++] = crs[6 + kshift];
            slicev[icx++] = crs[7 + kshift];
            slicev[icx++] = crs[0 + kshift];
            slicev[icx++] = crs[1 + kshift]; /*close*/
