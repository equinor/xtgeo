...
{
    std::array<double, 8> z_vals = { c.upper_sw.z(), c.upper_se.z(), c.upper_nw.z(),
                                     c.upper_ne.z(), c.lower_sw.z(), c.lower_se.z(),
                                     c.lower_nw.z(), c.lower_ne.z() };
    auto [min_it, max_it] = std::minmax_element(z_vals.begin(), z_vals.end());
                           ...
                           double min_dz_pinchout)
{
    ...
    // A cell is treated as pinched out (i.e. effectively inactive for the K-direction
    // connectivity) when its vertical thickness is below `min_dz_pinchout`.  Such
    // cells are skipped in direct K connections and bridged via pinch-out NNCs,
    // exactly like genuinely inactive cells.  A negative or zero threshold disables
    // this behaviour (only ACTNUM == 0 cells are bridged).
    auto cell_collapsed = [&](size_t i, size_t j, size_t k) {
        if (min_dz_pinchout <= 0.0)
            return false;
        auto cc = get_cell_corners_from_ijk(grid, i, j, k);
        return std::abs(cell_bot_z(cc) - cell_top_z(cc)) < min_dz_pinchout;
    };
    auto k_pinched = [&](size_t i, size_t j, size_t k) {
        return act(i, j, k) == 0 || cell_collapsed(i, j, k);
    };
    ...
    // Cells whose vertical thickness is below `min_dz_pinchout` are treated
    // as pinch-outs and handled by the NNC pass below.
    // ---------------------------------------------------------------
    for (size_t i = 0; i < nc; i++) {
        for (size_t j = 0; j < nr; j++) {
            for (size_t k = 0; k + 1 < nl; k++) {
                if (k_pinched(i, j, k) || k_pinched(i, j, k + 1))
                    continue;
    ...
    // ---------------------------------------------------------------
    // K pinch-out NNCs: within each column, connect the first non-pinched
    // cell above and below each pinched run (inactive OR collapsed cells).
    // ---------------------------------------------------------------
    for (size_t i = 0; i < nc; i++) {
        for (size_t j = 0; j < nr; j++) {
            for (size_t k = 0; k + 1 < nl; k++) {
                if (k_pinched(i, j, k))
                    continue;
                // Check if immediately below is pinched (inactive or collapsed)
                if (!k_pinched(i, j, k + 1))
                    continue;

                // Find next non-pinched cell below k
                size_t k2 = k + 2;
                while (k2 < nl && k_pinched(i, j, k2))
                    k2++;
                if (k2 >= nl)
                    continue;  // no non-pinched cell found below
