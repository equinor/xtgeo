                       ...
                       int use_region)
{
    ...
    for (size_t i = 0; i < grid_cpp.get_ncol(); i++) {
        for (size_t j = 0; j < grid_cpp.get_nrow(); j++) {
            ...
            khyb = nzhyb;
            for (int k = static_cast<int>(grid_cpp.get_nlay()); k >= 0; k--) {
                float zsum = 0.0f;
                ...
                // Store average depth for last active cell
                if (k > 0 && actnumsv_(i, j, k - 1) > 0 && flag_bot) {
                    zbot = 0.25f * zsum;
                    flag_bot = false;
                }

                // Copy actnum for original layers (from bottom)
                if (k > 0 && khyb > 0) {
                    // This is the corrected part. It places the original grid's bottom
                    // layers into the upper section of the hybrid grid's layers,
                    // after the intermediate horizontal layers.
