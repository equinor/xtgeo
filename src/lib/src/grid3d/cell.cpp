                    ...
                    int coord_axis)
{
    ...
    if (coord_axis >= 0 && coord_axis <= 2) {
        ...
        // OPM-compatible TPFA half-distances: d_i = |fc_i - cc_i|² / |n · (fc_i - cc_i)|.
        // Equivalent to the OPM formula  T_½ = K * A * (n · d_vec) / (d_vec · d_vec)
        // which correctly down-weights faces whose normal is misaligned with the
