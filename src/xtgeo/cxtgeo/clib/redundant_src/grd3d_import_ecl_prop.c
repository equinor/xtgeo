 ...
 * dvector_v            Array of double; will be filled.
 *                      NB stores also double and ints!
 * nktype               Returning the _type_ the _actual_ keys found,
 *                      1=int, 2=float/real, 3=double, -1=means undef
 *                      (ie keyword not found)
 *                      nktype[0]=2 means that first keyword asked for
 *                      is FLOAT (or REAL)
 * norder               Returning the actual order. For instance, the input list
 *                      0=PORO,1=PERMX,2=PORV,3=PERMY. This can for example
 *                      return the actual "found-in-order" list:
 *                      norder[0]=2,norder[1]=3,norder[2]=0, etc. If keyword
 *                      no 5 is not found, we get norder[5]=-1.
 *                      NOTE! for restart data (keywords with a date):
 *                      Both nktype and norder will be repeated per date
 *
 * dsuccess             Return a list of success of dates, eg (1,-1,-1) meaning
 *                      that first date request was found, but not the two next
			     ...
			     )
{
    ...
    int     *tmp_int_v, aday=0, amonth=0, ayear=0, nkfound=0, nrfound=0;
    ...
    /*
     *--------------------------------------------------------------------------
     * Show input (debug mode)
     *--------------------------------------------------------------------------
     */
    if (debug>2) {
	xtg_speak(s,3,"INPUT to routine %s:",s);
	xtg_speak(s,3,"FTYPE = %d", ftype);
	xtg_speak(s,3,"NXYZ = %d", nxyz);
	xtg_speak(s,3,"NDATES = %d", ndates);
	xtg_speak(s,3,"NKLIST = %d", nklist);
	xtg_speak(s,3,"KLIST = %s", klist);
	for (nn=0; nn<ndates; nn++) {
	    xtg_speak(s,3,"DATE no %d is = %d %d %d", nn, day[nn],
		      month[nn], year[nn]);
    ...
    /*
     *--------------------------------------------------------------------------
     * Process the keyword list; along list I need to break up into words
	...
	nktype[i]=-1;
	norder[i]=-1;
	xtg_speak(s,1,"Initial order for property element %d is %d",
		  i,norder[i]);
    }


    /* initialize qfoundkw list */
	...
	if (strncmp(cname, "SEQNUM  ",8) == 0) {
		...
		/* have now looked for props in last round; see if all found */
		for (nn=0; nn<nklist; nn++) {
		    if (qfoundkw[nn]==0) {
			if (ndates>0) {
			    sprintf(xtxt," (for the given date %d %d %d)",
				    ayear, amonth, aday);
			}
			else{
			    sprintf(xtxt,"...");
			}
			xtg_error(s,"STOP! Keyword %s is not found %s",
				  tmp_keywords[nn], xtxt);
		    }
		}
	 ...
	 * ---------------------------------------------------------------------
	 * The RESTART LOOP
	 * See if dates are matching, and if so, turn on "look for
	 * property flag": iproplook
	 * ---------------------------------------------------------------------
	 */

	if (ndates==0) nrfound=1;
	...
	if (ndates>0 && iproplook==0) {
		...
		for (i=0; i<ndates; i++) {
		    if (day[i] == aday && month[i] == amonth &&
			year[i] == ayear) {

			iproplook=1;
			xtg_speak(s,1,"Date match found for %d %d %d",
	...
	if (iproplook==1 && nkfound<nklist) {
	    /* thisfound=0;	  */
	    xtg_speak(s,1,"Property to check for is <%s>",cname);

	    xtg_speak(s,1,"Loop input keyword list...");

	    for (nn=0; nn<nklist; nn++) {

		strcpy(keyword,tmp_keywords[nn]);
		xtg_speak(s,2,"Looking for match keyword <%s> with <%s> ",
			  keyword, cname);
		if (strncmp(cname, keyword,8) == 0) {

		    xtg_speak(s,1,"--> Found keyword: <%s>",cname);
			...
			i=0;
			for (ib=0;ib<nxyz;ib++) {
			    /* only active cells stored normal for PORO etc*/
			    if (nact<nxyz) {
				if (p_actnum_v[ib]==1) {
	    ...
	    /* if (thisfound == 0) { */
	    /* 	xtg_error(s,"Did not find any requested keyword (for this timestep)"); */
	    /* } */
	...
	if (nrfound==ndates && nkfound==nklist) {
	    ...
	    xtg_speak(s,1,"NRFOUND %d   NKFOUND %d", nrfound, nkfound);
	    xtg_speak(s,1,"TOTALFOUND %d", totalfound);

	    if (nrfound*nkfound != totalfound) {
		xtg_error(s,"Some error, probably is a keyword missing for given "
			  "restart. Please check all data carefully");
	    }
	    break;
	}
    ...
    for (i=0;i<ndates;i++) {
	if (dsuccess[i]==0) {
	    xtg_speak(s,1,"Date did not match anywhere: "
		      "day=%d month=%d year=%d",day[i],month[i],year[i]);
    ...
    for (i=0;i<ndates;i++) {
	if (dsuccess[i]==1) {
	    xtg_speak(s,2,"Date: day=%d month=%d year=%d:",
		      day[i],month[i],year[i]);

	    for (nn=0; nn<nklist; nn++) {
		strcpy(keyword,tmp_keywords[nn]);
		xtg_speak(s,2,"Keyword is <%s>, type is <%d> and order is <%d>",
			  keyword,nktype[nn],norder[nn]);
	    }
	}
    }
