...
def _export_rms_ascii_blockedwells(
    ...
    """
    from xtgeo.io._welldata import WellData, WellLog
    ...
    with bwfile_wrapper.get_text_stream_write() as fhandle:
        for i, blockedwell in enumerate(blockedwells._wells):
            ...
            # Convert BlockedWell to BlockedWellData
            blockedwelldata = _blockedwell_io_factory._blockedwell_to_blockedwelldata(
                blockedwell
            )

            # Inject I/J/K indices as logs so they are written to the file.
            # BlockedWellData stores them separately from .logs; the generic
            # write functions only iterate over .logs, so we build a temporary
            # WellData that includes the three index columns.
            index_logs = (
                WellLog(
                    name="I_INDEX",
                    values=blockedwelldata.i_index,
                    is_discrete=False,
                    code_names=("UNK", "lin"),
                ),
                WellLog(
                    name="J_INDEX",
                    values=blockedwelldata.j_index,
                    is_discrete=False,
                    code_names=("UNK", "lin"),
                ),
                WellLog(
                    name="K_INDEX",
                    values=blockedwelldata.k_index,
                    is_discrete=False,
                    code_names=("UNK", "lin"),
                ),
            )
            temp_well = WellData(
                name=blockedwelldata.name,
                xpos=blockedwelldata.xpos,
                ypos=blockedwelldata.ypos,
                zpos=blockedwelldata.zpos,
                survey_x=blockedwelldata.survey_x,
                survey_y=blockedwelldata.survey_y,
                survey_z=blockedwelldata.survey_z,
...
def _export_csv_blockedwells(
    ...
    """Export multiple blocked wells to CSV format, as one file.
...
def blockedwells_to_stacked_file(
    ...
    if fmt == FileFormat.RMSWELL:
        ...
        raise InvalidFileFormatError(
            f"File format {fformat} is not supported for stacked BlockedWells export. "
...
def _import_rms_ascii_blockedwells(
    ...
    """Import multiple blocked wells from stacked RMS ASCII format.
    ...
    # Split on blank lines — each non-empty chunk is one complete blocked well
    for chunk in re.split(r"\n[ \t]*\n", text):
        chunk = chunk.strip()
        if not chunk:
            continue

        blockedwelldata = read_rms_ascii_blockedwell(io.StringIO(chunk))
        ...
        logger.debug(
            ...
            blockedwelldata.name,
            blockedwelldata.n_records,
            len(blockedwelldata.logs),
        )
