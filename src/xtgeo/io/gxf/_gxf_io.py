...
a regular, rotated grid with values at each node.
...
#KEY + a single value (the value is on the next line after KEY)
    - A KEY preceeded with a single "#" means that the KEY is part
    of the GFX specification
    - The GFX format does not allow value in quotes. However, some files
...
About the #GRID section
    ...
    - All lines in the #GRID section must contain only values;
        no keys, comments or free text.
...
Non-finite scalar metadata values, including #DUMMY, are rejected.
...
class GXFData:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(values._mask, np.ndarray):  # type: ignore[attr-defined]
    ...
    def _parse_gxf(cls, stream: Iterable[str], fileref_errmsg: str) -> Self:
        ...
        for line in lines:
            ...
            if TextParser.starts_with_prefix(line, "##"):
                ...
                if value_line is None or TextParser.starts_with_prefix(value_line, "#"):
                    raise ValueError(
                        f"In file {fileref_errmsg}: Missing value for "
                        f"key '{extension_key}'.\n"
                        f"Ill-formed header is not accepted."
                    )
                value_error_code = TextParser.is_single_finite_base10_value(value_line)
                if (
                    value_error_code
                    is TextParser.NumericValueErrorCode.NOT_SINGLE_NUMBER
                ):
                    raise ValueError(
                        f"In file {fileref_errmsg}: Invalid value for "
                        f"key '{extension_key}'. Expected a single finite "
                        "base-10 number."
                    )
                if value_error_code is TextParser.NumericValueErrorCode.NOT_BASE10:
                    raw_value = value_line[0]
                    raise ValueError(
                        f"In file {fileref_errmsg}: Invalid value '{raw_value}' "
                        f"for key '{extension_key}'. Expected a single finite "
                        "base-10 number."
                    )
                if value_error_code is TextParser.NumericValueErrorCode.NOT_FINITE:
                    raw_value = value_line[0]
                    raise ValueError(
                        f"In file {fileref_errmsg}: Non-finite value '{raw_value}' "
                        f"for key '{extension_key}' is not allowed."
                    )
                continue
            ...
            if key == "GRID":
                ...
                for grid_line in lines:
                    ...
                    for token in grid_line:
                        if not TextParser.is_base10_number(token):
                            raise ValueError(
                                f"In file {fileref_errmsg}: Invalid value '{token}' "
                                "inside #GRID section. Only finite base-10 "
                                "numbers are allowed."
                            )
                        try:
                            grid_value = float(token)
                        ...
                        if not np.isfinite(grid_value):
                            raise ValueError(
                                f"In file {fileref_errmsg}: Non-finite value "
                                f"'{token}' inside #GRID section is not allowed."
                            )
            ...
            if key == "SENSE":
                ...
                if (
                    len(value_line) != 1
                    or TextParser.strip_surrounding_quotes(value_line[0]) != "0"
                ):
                    raise ValueError(
            ...
            value_error_code = TextParser.is_single_finite_base10_value(value_line)
            if value_error_code is TextParser.NumericValueErrorCode.NOT_SINGLE_NUMBER:
                raise ValueError(
                    f"In file {fileref_errmsg}: Invalid value for "
                    f"key '#{key}'. Expected a single finite base-10 number."
                )
            if value_error_code is TextParser.NumericValueErrorCode.NOT_BASE10:
                raise ValueError(
            ...
            if value_error_code is TextParser.NumericValueErrorCode.NOT_FINITE:
                raise ValueError(
                    f"In file {fileref_errmsg}: Non-finite value '{raw_value}' "
                    f"for key '#{key}' is not allowed."
                )

            value_txt = TextParser.strip_surrounding_quotes(raw_value)
            try:
            ...
            if key in float_keys | {"DUMMY"} and not np.isfinite(parsed_value):
                raise ValueError(
                    f"In file {fileref_errmsg}: Non-finite value '{raw_value}' "
                    f"for key '#{key}' is not allowed."
                )
    ...
    def to_file(
        ...
        with wrapped_file.get_text_stream_write(encoding=encoding) as stream:
            ...
            if self.dummy is not None:
                ...
                stream.write("\n")
            elif np.ma.count_masked(self.grid):
                raise ValueError("Cannot write masked GXF grid without a dummy value.")
