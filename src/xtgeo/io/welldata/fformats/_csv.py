...
def write_csv_well(
    ...
    if is_stream:
        ...
        filepath = Path(filepath)
        logger.info("Writing well data to CSV: %s", filepath)
...
def read_csv_blockedwell(
    ...
    required_cols = [xname, yname, zname, i_col, j_col, k_col, wellname_col]
    well_data = _read_and_filter_csv(filepath, wellname, wellname_col, required_cols)
    ...
    i_index = well_data[i_col].to_numpy(dtype=np.float64)
    j_index = well_data[j_col].to_numpy(dtype=np.float64)
    k_index = well_data[k_col].to_numpy(dtype=np.float64)
    ...
    # Extract logs
    excluded_cols = [xname, yname, zname, i_col, j_col, k_col, wellname_col]
    logs = _extract_logs(well_data, excluded_cols)
...
def write_csv_blockedwell(
    ...
    if is_stream:
        ...
        filepath = Path(filepath)
        logger.info("Writing blocked well data to CSV: %s", filepath)

    data = {
        xname: blocked_well.survey_x,
        yname: blocked_well.survey_y,
        zname: blocked_well.survey_z,
        i_col: blocked_well.i_index,
        j_col: blocked_well.j_index,
        k_col: blocked_well.k_index,
