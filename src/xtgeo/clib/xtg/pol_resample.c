             ...
             int debug)
{
    ...
    xtgverbose(debug);
    xtg_speak(s, 2, "Running with sampling %f, first entry is %f %f %f", smpl, xv[0],
              yv[0], zv[0]);
    ...
    ier = pol_geometrics(xv, nlen, yv, nlen, zv, nlen, tmp_tlen, nlen, tmp_dtlen, nlen,
                         tmp_hlen, nlen, tmp_dhlen, nlen, debug);
    ...
    if (debug > 2) {
        for (i = 0; i < n; i++) {
            xtg_speak(s, 3, "Pre coords: N I %d %d .. %f %f %f", n, i, xov[i], yov[i],
                      zov[i]);
        }
    }
    ...
    if (debug > 2) {
        for (i = 0; i < n; i++) {
            xtg_speak(s, 3, "ALONG WELL coords: N I %d %d .. %f %f %f", n, i, xov[i],
                      yov[i], zov[i]);
        }
    }
    ...
    if (nnext > 0) {
        if (debug > 1)
            xtg_speak(s, 2, "Last point on well is %f %f %f: ", x1, y1, z1);
        z0 = z1;
        ...
        for (i = 0; i <= nnext; i++) {
            ier = x_vector_linint2(x10, y10, z0, x11, y11, z1,
        ...
        if (n >= nbuf) {
            xtg_error(s, "Something went wrong in %s: NBUF = %d too small", s, nbuf);
        }
    }
    ...
    if (debug > 1) {
        for (i = 0; i < n; i++) {
            ...
            xtg_speak(s, 2, "ALLX coords ::: I %d .. %f %f %f  dxy = %6.2f", n, i,
                      xov[i], yov[i], zov[i], delta);
        }
    }
    ...
    *nolen = n;
    ...
    if (ier != 0) {
        xtg_error(s, "Something went wrong in gemetrics for %s IER = %d", s, ier);
    }
