    ...
    for (m = mtopmark; m <= mbotmark; m++) {
        ...
        if (lzone >= zlmin && lzone <= zlmax) {
            ...
            */
            ...
            long ib = grd3d_point_in_cell(ibstart2, 0, x, y, z, nx, ny, 1, coordsv,
            ...
            if (outside == 0) {
                ...
                /* loop cells in simplified (one layer) grid */
                ib =
                  grd3d_point_in_cell(ibstart, 0, x, y, z, nx, ny, nz, coordsv, zcornsv,
                                      actnumsv, maxradsearch, sflag, &nradsearch, 0);
