...
class BoundingBox3D:
    ...
    def __init__(self, values: Iterable[float]) -> None:
        ...
        if not isinstance(values, Iterable):
            raise ValueError("Expected an iterable of 6 float values")
    ...
    def describe(self, flush: bool = True) -> str:
        """
        Return a string description of the bounding box.
        ...
        dsc = XTGDescription()
        dsc.title("Description of BoundingBox3D instance")
        dsc.txt("Object ID", id(self))
        if self.is_valid():
            dsc.txt("(x_min, x_max, y_min, y_max, z_min, z_max): ")
            dsc.txt(str(self._values))
        else:
            dsc.txt("BoundingBox3D:", "Empty or invalid.")
