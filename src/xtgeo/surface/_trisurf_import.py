...
def import_tsurf(mfile: FileWrapper) -> TriangulatedSurfaceDict:
    ...
    tsurf_data = TSurfData.from_file(mfile.file)

    # Uses 0-based indexing, so we need to convert from the 1-based
    ...
    if tsurf_data.coord_sys is not None:
        data["free_form_metadata"] = {"tsurf_coord_sys": asdict(tsurf_data.coord_sys)}
