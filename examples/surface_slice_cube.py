...
def slice_a_cube_with_surface():
    ...
    # must match)
    mysurf = xtgeo.surface_from_file(surfacefile, fformat='ijxyz',
                                     template=mycube)
...
def attribute_around_surface_symmetric():
    ...
    myattrs = mysurf.slice_cube_window(mycube, attribute=attrs,
                                       sampling='trilinear',
                                       zrange=10.0)
    for attr in myattrs.keys():
        myattrs[attr].to_file('myfile_symmetric_' + attr + '.dat',
                              fformat='ijxyz')
...
def attribute_around_surface_asymmetric():
    ...
    myattrs = mysurf.slice_cube_window(mycube, attribute=attrs,
                                       sampling='trilinear',
                                       zsurf=sabove, other=sbelow)
    for attr in myattrs.keys():
        if DEBUG:
            myattrs[attr].describe()

        myattrs[attr].to_file('myfile_asymmetric_' + attr + '.dat',
                              fformat='ijxyz')
...
def attribute_around_constant_cube_slices():
    ...
    myattrs = sabove.slice_cube_window(mycube, attribute=attrs,
                                       sampling='trilinear',
                                       zsurf=sabove, other=sbelow)
    for attr in myattrs.keys():
        if DEBUG:
        ...
        myattrs[attr].to_file('myfile_constlevels_' + attr + '.dat',
                              fformat='ijxyz')
