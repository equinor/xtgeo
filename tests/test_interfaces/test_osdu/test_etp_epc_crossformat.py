...
DROGON_GRID_UUID = "8c4f84a5-d819-4ae9-b7e4-03fe06ca5ae7"  # Geogrid (faulted)
...
def test_rddms_epc_rddms_roundtrip(tmp_path):
    ...
    with EpcFileProvider(epc_path, mode="w") as epc_w:
        ...
        print(f"  Wrote grid: {grid_epc_uuid} (new UUID)")
        assert grid_epc_uuid != SLEIPNER_GRID_UUID, "Expected new UUID"
    ...
    print(f"  Wrote grid: {grid_etp_uuid} (new UUID)")
    assert grid_etp_uuid != grid_epc_uuid, "Expected new UUID at each step"
...
def test_drogon_faulted_grid_roundtrip(tmp_path):
    """Cross-format roundtrip for Drogon faulted grid (split coordinate lines).

    RDDMS (maap/drogon) → xtgeo → EPC → xtgeo → RDDMS (maap/xtgeo) → xtgeo.
    """
    from xtgeo.interfaces.osdu import EpcFileProvider, EtpProvider
    from xtgeo.interfaces.osdu._ijk_grid import ijk_grid_to_xtgeo, xtgeo_grid_to_resqml

    # =======================================================================
    # Step 1: Read from RDDMS (maap/drogon) → xtgeo
    # =======================================================================
    print("\n=== Step 1: RDDMS (maap/drogon) → xtgeo ===")
    config_src = _etp_config("maap/drogon")
    p_src = EtpProvider(config_src)
    p_src.open()

    grid_orig, props_orig = ijk_grid_to_xtgeo(
        p_src, DROGON_GRID_UUID, load_properties=False
    )
    print(
        f"  Grid: {grid_orig.ncol}×{grid_orig.nrow}×{grid_orig.nlay}, "
        f"active={grid_orig._actnumsv.sum()}/{grid_orig._actnumsv.size}"
    )
    p_src.close()
    ...
    # Verify splits exist
    zcorn_4d = zcorn_orig.reshape(
        grid_orig.ncol + 1, grid_orig.nrow + 1, grid_orig.nlay + 1, 4
    )
    n_split_nodes = np.any(zcorn_4d != zcorn_4d[:, :, :, 0:1], axis=3).sum()
    print(f"  Split nodes: {n_split_nodes}")
    assert n_split_nodes > 0, "Expected faulted grid with split Z-values"

    # =======================================================================
    # Step 2: xtgeo → EPC file
    # =======================================================================
    print("\n=== Step 2: xtgeo → EPC file ===")
    epc_path = str(tmp_path / "drogon_roundtrip.epc")

    with EpcFileProvider(epc_path, mode="w") as epc_w:
        uuids = xtgeo_grid_to_resqml(
            epc_w,
            grid_orig,
            title="Drogon_RT_Grid",
            preserve_uuids=False,
        )
        grid_epc_uuid = uuids["Drogon_RT_Grid"]
        print(f"  Wrote grid: {grid_epc_uuid} (new UUID)")
        assert grid_epc_uuid != DROGON_GRID_UUID, "Expected new UUID"

    h5_size = Path(epc_path.replace(".epc", ".h5")).stat().st_size / 1024 / 1024
    ...
    print("\n=== Step 3: EPC file → xtgeo ===")
    with EpcFileProvider(epc_path, mode="r") as epc_r:
        grid_epc, _ = ijk_grid_to_xtgeo(epc_r, grid_epc_uuid, load_properties=False)
        print(f"  Grid: {grid_epc.ncol}×{grid_epc.nrow}×{grid_epc.nlay}")
    ...
    uuids2 = xtgeo_grid_to_resqml(
        p_dst,
        grid_epc,
        title="Drogon_RT_Grid_ETP",
        preserve_uuids=False,
    )
    grid_etp_uuid = uuids2["Drogon_RT_Grid_ETP"]
    print(f"  Wrote grid: {grid_etp_uuid} (new UUID)")
    assert grid_etp_uuid != grid_epc_uuid, "Expected new UUID at each step"
    p_dst.close()
    ...
    grid_final, _ = ijk_grid_to_xtgeo(
        p_final, grid_etp_uuid, load_properties=False
    )
    print(
        f"  Grid: {grid_final.ncol}×{grid_final.nrow}×{grid_final.nlay}, "
        f"active={grid_final._actnumsv.sum()}/{grid_final._actnumsv.size}"
    )
    p_final.close()
    ...
    if np.array_equal(coord_orig, coord_final):
        ...
        max_diff = np.max(
            np.abs(coord_orig.astype(np.float64) - coord_final.astype(np.float64))
        )
        n_diff = np.sum(coord_orig != coord_final)
        print(
            f"  coord: DIFFER (max_diff={max_diff:.2e}, n_diff={n_diff}/{coord_orig.size})"
        )
        np.testing.assert_allclose(
            coord_final.astype(np.float64),
            coord_orig.astype(np.float64),
    ...
    if np.array_equal(zcorn_orig, zcorn_final):
        ...
        max_diff = np.max(
            np.abs(zcorn_orig.astype(np.float64) - zcorn_final.astype(np.float64))
        )
        n_diff = np.sum(zcorn_orig != zcorn_final)
        print(
            f"  zcorn: DIFFER (max_diff={max_diff:.2e}, n_diff={n_diff}/{zcorn_orig.size})"
        )
        np.testing.assert_allclose(
            zcorn_final.astype(np.float32),
            zcorn_orig.astype(np.float32),
    ...
    # Verify splits are preserved through the roundtrip
    zcorn_final_4d = zcorn_final.reshape(
        grid_final.ncol + 1, grid_final.nrow + 1, grid_final.nlay + 1, 4
    )
    n_split_final = np.any(
        zcorn_final_4d != zcorn_final_4d[:, :, :, 0:1], axis=3
    ).sum()
    print(f"  Split nodes preserved: {n_split_final} (original: {n_split_nodes})")

    print("\n=== PASS: Drogon faulted grid roundtrip complete ===")
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as td:
        td_path = Path(td)
        print("=" * 60)
        print("SLEIPNER (unfaulted parametric grid + surface)")
        print("=" * 60)
        test_rddms_epc_rddms_roundtrip(td_path)
        print()
        print("=" * 60)
        print("DROGON (faulted parametric grid, split coordinate lines)")
        print("=" * 60)
        test_drogon_faulted_grid_roundtrip(td_path)
