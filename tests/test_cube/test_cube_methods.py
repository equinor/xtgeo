...
def test_get_xy_value_from_ij_clib_error_raises(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test that get_xy_value_from_ij surfaces C-library failures.

    What is tested:
        ``get_xy_value_from_ij`` is called with the low-level XY conversion
        patched to return a non-zero status.

    Expected behaviour:
        An ``XTGeoCLibError`` is raised with the failing code.
    """
    cube = Cube(ncol=3, nrow=3, nlay=2, xinc=1, yinc=1, zinc=1)

    monkeypatch.setattr(
        "xtgeo.cube._cube_utils._cxtgeo.cube_xy_from_ij",
        lambda *args: (9, 0.0, 0.0),
    )

    with pytest.raises(XTGeoCLibError, match="error code: 9"):
        cube.get_xy_value_from_ij(1, 1)
...
def test_resample_clib_error_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    """Test that resample raises on non-warning C-library errors.

    What is tested:
        ``resample`` is called with the low-level routine patched to return a
        non-zero, non-warning status.

    Expected behaviour:
        An ``XTGeoCLibError`` is raised.
    """
    cube = Cube(ncol=2, nrow=2, nlay=2, xinc=1, yinc=1, zinc=1)
    other = Cube(ncol=2, nrow=2, nlay=2, xinc=1, yinc=1, zinc=1)
    ...
    with pytest.raises(XTGeoCLibError, match="cube_resample_cube failed"):
        cube.resample(other)
...
def test_cube_from_roxar_sets_metadata_required(
    ...
    Expected behaviour:
        A Cube instance is returned and ``metadata.required`` points to itself.
    """
    monkeypatch.setattr(
...
def test_find_long_line_keeps_longest_available_vector() -> None:
    ...
    """
    result = cube_import._find_long_line({"a": [2, 3], "b": [5, 6]}, nany=10)
