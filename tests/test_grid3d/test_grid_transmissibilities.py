...
class TestReturnTypes:
    ...
    def computed_results(self):
        ...
        return _compute(3, 4, 5)

    def test_tranx_is_gridproperty(self, computed_results):
        tranx, trany, tranz, nnc = computed_results
        assert isinstance(tranx, xtgeo.GridProperty)

    def test_trany_is_gridproperty(self, computed_results):
        tranx, trany, tranz, nnc = computed_results
        assert isinstance(trany, xtgeo.GridProperty)

    def test_tranz_is_gridproperty(self, computed_results):
        tranx, trany, tranz, nnc = computed_results
        assert isinstance(tranz, xtgeo.GridProperty)

    def test_nnc_is_dataframe(self, computed_results):
        tranx, trany, tranz, nnc = computed_results
        assert isinstance(nnc, pd.DataFrame)

    def test_nnc_has_expected_columns(self, computed_results):
        tranx, trany, tranz, nnc = computed_results
...
class TestUnitTran:
    ...
    def test_tranx_all_one(self, computed_results):
        tranx, _, _, _ = computed_results
        # Exclude the dummy last I-slice (padded with 0)
        assert np.all(
            np.isclose(tranx.values.filled(np.nan)[:-1, :, :], _C_METRIC, rtol=1e-6)
        )

    def test_trany_all_one(self, computed_results):
        _, trany, _, _ = computed_results
        # Exclude the dummy last J-slice (padded with 0)
        assert np.all(
            np.isclose(trany.values.filled(np.nan)[:, :-1, :], _C_METRIC, rtol=1e-6)
        )

    def test_tranz_all_one(self, computed_results):
        _, _, tranz, _ = computed_results
...
# ---------------------------------------------------------------------------
# Scalar input for permeability and NTG
# ---------------------------------------------------------------------------


class TestScalarInput:
    """permx, permy, permz and ntg can be passed as scalar floats.

    Scalar inputs should be equivalent to a uniformly-filled GridProperty
    with the same value.
    """

    @pytest.fixture(scope="class")
    def grid(self):
        return _box_grid(3, 4, 5)
    ...
    def test_scalar_perm_equivalent_to_gridproperty(self, grid):
        """Scalar permx=2.0 produces same TRANX as uniform GridProperty 2.0."""
        px_scalar, py_scalar, pz_scalar = 2.0, 3.0, 4.0
        px = _uniform_prop(grid, px_scalar, "permx")
        ...
        assert np.allclose(
            ...
            tx_p.values.filled(np.nan)[:-1, :, :],
            atol=1e-12,
        )
        assert np.allclose(
            ty_s.values.filled(np.nan)[:, :-1, :],
            ty_p.values.filled(np.nan)[:, :-1, :],
            atol=1e-12,
        )
        assert np.allclose(
            tz_s.values.filled(np.nan)[:, :, :-1],
            tz_p.values.filled(np.nan)[:, :, :-1],
            atol=1e-12,
        )

    def test_scalar_ntg_equivalent_to_gridproperty(self, grid):
        """Scalar ntg=0.5 produces same TRAN as uniform GridProperty 0.5."""
        ntg_scalar = 0.5
        ntg_prop = _uniform_prop(grid, ntg_scalar, "ntg")
    ...
    def test_mixed_scalar_and_gridproperty(self, grid):
        ...
        # trany: k_eff = 3.0 * 0.5 = 1.5 → T = 1.5 * _C_METRIC
        assert np.all(
            np.isclose(
                trany.values.filled(np.nan)[:, :-1, :], 1.5 * _C_METRIC, rtol=1e-6
            )
        )
        # tranz: k_eff = 4.0 (no ntg) → T = 4.0 * _C_METRIC
        assert np.all(
            np.isclose(
                tranz.values.filled(np.nan)[:, :, :-1], 4.0 * _C_METRIC, rtol=1e-6
            )
...
class TestInactiveCells:
    @pytest.fixture(scope="class")
    def all_active_results(self):
        """Compute results for fully active grid, reused across tests."""
        return _compute(3, 4, 5)
    ...
    def test_all_active_no_masked_entries(self, all_active_results):
        """A fully grid should have no masked TRAN values (excl. dummy slices)."""
        tranx, trany, tranz, _ = all_active_results
...
class TestConformingGridNoNNC:
    ...
    def test_empty_nnc_for_box_grid(self, computed_results):
        _, _, _, nnc = computed_results
        assert len(nnc) == 0

    def test_empty_nnc_dataframe_has_correct_columns(self, computed_results):
        _, _, _, nnc = computed_results
...
class TestNNCTypeStrings:
    ...
    def test_type_values_are_valid_strings(self, computed_results):
        _, _, _, nnc = computed_results
    ...
    def test_empty_nnc_type_column_dtype(self, computed_results):
        _, _, _, nnc = computed_results
...
class TestMinDzPinchout:
    ...
    def default_results(self):
        """Compute with default min_dz_pinchout, reused across tests."""
        return _compute(3, 4, 5)
    ...
    def test_default_value_accepted(self, default_results):
        """Default min_dz_pinchout=1e-4 runs without error."""
        _, _, _, nnc = default_results
        assert isinstance(nnc, pd.DataFrame)
...
class TestCompareCases:
    """Test the B7 case with numbers from OPM flow, to check for consistency."""
    ...
    def test_drogon_opm_transm_rh(self, testdata_path):
        ...
        np.testing.assert_allclose(tranx_a, tranx_r, rtol=0.20, atol=1e-12)
        np.testing.assert_allclose(trany_a, trany_r, rtol=0.20, atol=1e-12)
...
class TestEmeraldOriginal:
    ...
    def test_emerald_original_nnc_vs_vendor(self, testdata_path):
        """Validate fault NNCs against reference values from a vendor tool (OPM flow).
