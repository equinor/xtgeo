...
def _build_emerald_boundary_df(testdata_path, region_o):
    ...
    NestReg=2 cell always in I1/J1/K1, NestReg=1 mother always in I2/J2/K2.
    DIRECTION is from the NestReg=2 cell toward the NestReg=1 cell.)
    """
    import pandas as pd
    ...
    def _add(r1, r2, i1b, j1b, k1b, i2b, j2b, k2b, t_val, direction):
        """Append a row with NestReg=2 cell always first."""
        if t_val <= 0:
            return
        if r1 == 2:
    ...
    for row in nncs_fault.itertuples():
        ...
        if {r1, r2} != {1, 2}:
            continue
        # Direction from NestReg=2 cell toward NestReg=1 cell
        ...
        dvec = (
            int(np.sign(moth_i - hole_i)),
            int(np.sign(moth_j - hole_j)),
            int(np.sign(moth_k - hole_k)),
        )
...
class TestBoxGrid:
    ...
    The single NNC connects cell (i=3, 1-based: I1=4) to cell (i=1, I2=2).
    ...
    def test_i_direction_single_nnc_found(self):
        ...
        _, _, _, _, nnc_df, _ = grid.get_transmissibilities(
            perm, perm, perm, ntg, nested_id_property=nested, search_radius=5.0
        )

        assert len(nnc_df) == 1
        row = nnc_df.iloc[0]
        assert row["I1"] == 4  # refined cell i=3 (1-based)
        assert row["I2"] == 2  # mother cell i=1 (1-based)
    ...
    def test_no_hole_returns_empty_dataframe(self):
        ...
        nested = xtgeo.GridProperty(
            grid,
            values=np.ones((3, 1, 1), dtype=np.int32),
            name="NEST_ID",
            discrete=True,
        )
        perm = _uniform(grid, 1.0, "perm")
        ...
        _, _, _, _, nnc_df, rbnd = grid.get_transmissibilities(
            perm, perm, perm, ntg, nested_id_property=nested
        )
    ...
    def test_ntg_scales_ij_transmissibility(self):
        ...
        _, _, _, _, nnc_df, _ = grid.get_transmissibilities(
            perm, perm, perm, ntg_half, nested_id_property=nested, search_radius=5.0
        )
    ...
    def test_transmissibility_proportional_to_perm(self):
        ...
        _, _, _, _, nnc1, _ = grid.get_transmissibilities(
            perm1, perm1, perm1, ntg, nested_id_property=nested, search_radius=5.0
        )

        perm2 = _uniform(grid, 2.0, "perm2")
        _, _, _, _, nnc2, _ = grid.get_transmissibilities(
            perm2, perm2, perm2, ntg, nested_id_property=nested, search_radius=5.0
        )
    ...
    def test_nested_drogon_nncs(self, testdata_path):
        ...
        def get_nncs():
            return grid.get_transmissibilities(
                permx, permy, permz, ntg, nested_id_property=nested
            )
...
class TestEmeraldNestedCase:
    ...
    def test_emerald_hybrid_onecell_no_refinement(self, testdata_path):
        ...
        _, _, _, _, nncs_connections, _ = hgrid.get_transmissibilities(
            ...
            permz_hybrid,
            ntg_hybrid,
            nested_id_property=region_hybrid,
        )
    ...
    def test_emerald_boundary_table_vs_hybrid_nncs(self, testdata_path):
        ...
        * **NNC**   – fault-plane NNCs from the original grid that cross the
        ...
        1. **TRANY where hole is at J+1** (southern boundary, `J2 > J1` in the
           table).  That edge is the fault plane itself.  The original grid
           computes a regular TPFA face-transmissibility there, while the hybrid
           uses the geometric-overlap NNC-scan algorithm, which distributes the
           T across multiple refined cells.  The per-entry T values therefore do
           *not* agree 1:1.

        2. **TRANX where hole is at I−1** (eastern boundary of NestReg=2, ``I2
           < I1`` in the table).  The NestedHybrid NNC generator currently does
           **not** produce correct large-T connections for mother cells whose
           adjacent hole face is in the −I direction.  These mother cells receive
           only spurious tiny-T NNCs.  This is a known gap to be fixed.
        """
        ...
        # ------------------------------------------------------------------
        # Hybrid grid: collect NestedHybrid NNCs, normalised so the NestReg=1
        # (mother) cell is always stored in the I2/J2/K2 columns (I1/J1/K1
        # is the refined cell, matching the convention in boundary_df).
        # ------------------------------------------------------------------
        ...
        _, _, _, _, nncs_h, _ = grid_h.get_transmissibilities(
            permx_h, permy_h, permz_h, ntg_h, nested_id_property=region_h
        )
        ...
        for row in nncs_h.itertuples():
            # I2/J2/K2 is always the mother (NestReg=1) in NestedHybrid output
            mother = (row.I2, row.J2, row.K2)
        ...
        to_check = boundary_df[
            ~(
                ...
                )  # TRANY southern boundary (J- direction)
                | (
                    boundary_df["I1"] < boundary_df["I2"]
                )  # TRANX eastern boundary (I- direction)
            )
        ]

        mismatches = []
        for brow in to_check.itertuples():
            mother = (brow.I2, brow.J2, brow.K2)
            t_o = brow.T
    ...
    def test_emerald_nnc_vs_boundary_by_i2j2k2_and_direction(self, testdata_path):
        """Join nncs_h and boundary_df on (I2, J2, K2, DIRECTION) and compare T.

        Both DataFrames use the same convention:
          I1/J1/K1 = NestReg=2 (refined/hole) cell
          I2/J2/K2 = NestReg=1 (mother) cell
          DIRECTION = direction from the refined cell toward the mother cell
        ...
        * ``DIRECTION == "I+"`` — hole is west of mother (TRANX western boundary
          of the NestReg=2 region).  The NNC algorithm does not produce correct
          large-T connections for these faces (documented gap, same exclusion as
          in ``test_emerald_boundary_table_vs_hybrid_nncs``).
        * ``DIRECTION == "J-"`` — hole is north of mother (TRANY southern
          boundary / fault plane).  The original grid uses a regular TPFA face-T
          while the hybrid uses the geometric-overlap scan, so per-entry values
          do not agree 1:1.
        * ``DIRECTION == "I-"`` for **corner hole cells only** — a corner hole
        ...
        # Inner join: only rows where (I2, J2, K2, DIRECTION) matches in both
        merged = boundary_df.merge(
            nncs_h[["I2", "J2", "K2", "DIRECTION", "T"]].rename(
                columns={"T": "trans_hyb"}
            ),
            on=["I2", "J2", "K2", "DIRECTION"],
            how="inner",
        ).rename(columns={"T": "trans_orig"})

        assert len(merged) > 0, "No rows matched on (I2, J2, K2, DIRECTION)"
        ...
        corner_holes = set(
            boundary_df.groupby(["I1", "J1", "K1"])["DIRECTION"]
            .nunique()
            .pipe(lambda s: s[s > 1].index.tolist())
        )
        corner_mask = merged[["I1", "J1", "K1"]].apply(tuple, axis=1).isin(corner_holes)
        ...
        assert bad.empty, (
            f"{len(bad)} T mismatches on (I2, J2, K2, DIRECTION) join:\n"
            + bad[["I2", "J2", "K2", "DIRECTION", "trans_orig", "trans_hyb"]]
            .head(15)
            .to_string()
        )
